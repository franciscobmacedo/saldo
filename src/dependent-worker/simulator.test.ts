...
import { Twelfths, LunchAllowance, defaultLunchAllowance, DependentWorkerResult } from "@/dependent-worker/schemas";
...
vi.mock("@/dependent-worker/calculations", () => ({
    getPartnerExtraDeduction: vi.fn((married: boolean, numberOfHolders: number | null | undefined, partnerDisabled: boolean) => {
    ...
    getDisabledDependentExtraDeduction: vi.fn((taxRetentionTable: ActualTaxRetentionTable, numDisabled: number) => {
...
vi.mock("@/tables/tax-retention", () => {
    class MockTaxBracket {
        ...
        calculate_tax(
            ...
            const basicTax = (taxable_income + twelfths_income) * 0.1 - extra_deduction - (number_of_dependents * 10);
    ...
    class MockTaxRetentionTable {
        ...
        constructor(public situationCode: string, public dependentDisabledDeductionVal?: number) {
            this.dependent_disabled_addition_deduction = dependentDisabledDeductionVal;
        }

        static load(date_start: Date, date_end: Date, location: ConfigSchemas.LocationT, situation_code: string) {
            return new MockTaxRetentionTable(situation_code, 30);
        }

        find_bracket(taxableIncome: number) {
            return new MockTaxBracket({ limit: taxableIncome + 1000, max_marginal_rate: 0.2 });
        }
    }
...
describe("simulateDependentWorker", () => {
    ...
    it("should calculate for a basic scenario with defaults", () => {
        ...
        expect(result.taxable_income).toBe(baseIncome + defaultLunchAllowance.taxable_monthly_value);
        expect(result.social_security_tax).toEqual(0.11);
        expect(result.lunch_allowance).toEqual(defaultLunchAllowance);
    });
    ...
    it("should calculate for a married individual, 1 holder, 2 dependents", () => {
        ...
        expect(result).toBeDefined();
        expect(result.taxable_income).toBe(incomeVal + defaultLunchAllowance.taxable_monthly_value);
    });

    it("should calculate with worker disability and custom lunch allowance", () => {
        const incomeVal = 1500;
        const customLunch: LunchAllowance = {
            daily_value: 8,
            mode: "salary",
            days_count: 22,
        ...
        const result = simulateDependentWorker({
            income: incomeVal,
            disabled: true,
            lunchAllowance: customLunch,
        });
        expect(result).toBeDefined();
        expect(result.lunch_allowance.daily_value).toBe(8);
        expect(result.taxable_income).toBe(incomeVal + customLunch.taxable_monthly_value);
    });
    ...
    it("should calculate for 'acores' location and no twelfths", () => {
        ...
        const expectedTaxable = incomeVal + defaultLunchAllowance.taxable_monthly_value;
        const expectedRetentionIncome = expectedTaxable + expectedTwelfthsIncome;
        expect(result.gross_income).toBe(expectedRetentionIncome + defaultLunchAllowance.tax_free_monthly_value)
    });
    ...
    it("should calculate yearly gross and net salaries correctly based on (mocked) monthly values", () => {
        ...
        const expectedYearlyLunchAllowance = defaultLunchAllowance.monthly_value * 11;
        const expectedYearlyGrossSalary = income * 14 + expectedYearlyLunchAllowance;
        const expectedYearlyNetSalary = result.net_salary * (14 - Number(twelfths));
