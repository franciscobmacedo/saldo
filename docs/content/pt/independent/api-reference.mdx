...
- **Rendimento do 1º Trimestre** (Jan, Fev, Mar) influencia os pagamentos à SS para Mai, Jun, Jul.
- **Rendimento do 2º Trimestre** (Abr, Mai, Jun) influencia os pagamentos à SS para Ago, Set, Out.
- **Rendimento do 3º Trimestre** (Jul, Ago, Set) influencia os pagamentos à SS para Nov, Dez, Jan.
- **Rendimento do 4º Trimestre** (Out, Nov, Dez) influencia os pagamentos à SS para Fev, Mar, Abr.

<RunCode defaultCode={`// Simular rendimento flutuante ao longo do ano
const variableEmployed = simulateIndependentWorker({
  income: [1500, 1800, 2000, 1200, 1000, 2500, 3000, 3200, 1500, 1000, 2800, 4000],
  expenses: 3000,
});

console.log('=== Detalhe de SS com Rendimento Variável ===');
console.log(\`SS Anual Estimada: €\${variableEmployed.ssPay.year.toFixed(2)}\`);
console.log(\`Cálculo da Base Mensal (Após Limite Central): [\${(variableEmployed.normalizedInternals.socialSecurity.baseMonthlyAfterCap as number[]).join(', ')}]\`);`} />
