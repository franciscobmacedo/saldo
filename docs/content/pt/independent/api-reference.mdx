...
APIs de trabalhador independente (regime simplificado/RNH). Para cálculos de processamento salarial, consulte a [API de trabalhador por conta de outrem](/pt/dependent/api-reference).
...
// 2. Analisar do formato exportado das Finanças e correr simulador:
const result = simulateFromGreenReceiptsCsv({
  csvContent, 
  currentTaxRankYear: 2025,
  expenses: 500, // Envie qualquer atributo regular da simulação
});

console.log(\`Rendimento líquido final: €\${result.netIncome.year.toFixed(2)}\`);
console.log(\`SS de Janeiro estimada? \${result.ssQ1Approximated}\`);
console.log(\`IRS a Pagar: €\${result.irsPay.year.toFixed(2)}\`);`} />
